...
public class HUDMenu extends IMenu {
    ...
    public function HUDMenu() {
        ...
        if (this.__animFactory_SafeRect_mcaf1 == null) {
            ...
            try {
                this.swfLoader.load(
                        new URLRequest("no_stealth_meter.swf"),
                        new LoaderContext(false, ApplicationDomain.currentDomain)
                );
                this.swfLoader.uncaughtErrorEvents.addEventListener(
                        UncaughtErrorEvent.UNCAUGHT_ERROR, this.uncaughtErrorHandler
                );
            } catch (e:Error) {
                this.displayError("no_stealth_meter.swf_Loader_ERROR: " + e.toString());
            }
        }
    }
    ...
    private function onHUDModeUpdate(arEvent:FromClientDataEvent):void {
        var mode:String = String(arEvent.data.hudMode);
    ...
    private function onMenuStackDataUpdate(arEvent:FromClientDataEvent):void {
        ...
        if (Boolean(arEvent.data) && Boolean(arEvent.data.menuStackA)) {
            ...
            if (Boolean(arEvent.data) && Boolean(arEvent.data.menuStackA)) {
                mapMenuOpen = false;
                for (i = arEvent.data.menuStackA.length - 1; i > -1; i--) {
    ...
    private function updateHUDNotificationsOffset():void {
        ...
        switch (this.m_LastHUDMode) {
            case HUDModes.DIALOGUE_MODE:
                ...
                break;
            case HUDModes.WORKSHOP_MODE:
                notificationsXOffset = NOTIFICATION_OFFSET_X_WORKSHOP;
                notificationsYOffset = this.m_IsFreeCamMode ? NOTIFICATION_OFFSET_Y_WORKSHOP_FREECAM : NOTIFICATION_OFFSET_Y_WORKSHOP;
                break;
    ...
    public function sendChatMessage(Message:String):* {
        ...
        if (this.CharacterInfoData) {
            Username = String(this.CharacterInfoData.name);
        }
